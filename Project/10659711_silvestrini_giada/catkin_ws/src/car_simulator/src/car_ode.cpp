...
car_ode::car_ode(double deltaT, tyreModel tyre_model) : dt(deltaT),
...
void car_ode::setInitialState(double r0, double beta0, double x0, double y0, double psi0)
{
    // Initial state values
    state[0] = r0;
    state[1] = beta0;
...
void car_ode::setVehicleParams(double m, double a, double b, double Cf, double Cr, double mu, double Iz)
{
...
void car_ode::setReferenceCommands(double velocity, double steer)
{
    Vx    = velocity;
    delta = steer;
}

void car_ode::integrate()
{
...
void car_ode::vehicle_ode(const state_type &state, state_type &dstate, double t)
{
    ...
    double zf = std::tan(alphaf);   // theoretical longitudinal slip
    double zr = std::tan(alphar);   // theoretical longitudinal slip
    double zf_sl = 3*mu*Fzf/Cf;     // minimum slip value that gives full sliding at the front
    double zr_sl = 3*mu*Fzr/Cr;     // minimum slip value that gives full sliding at the rear
    ...
    //ROS_INFO("Fyf: %f -- Fyr: %f", Fyf, Fyr);
    
    // Vehicle equations
    //dstate[0] = (a*(Fxf*std::sin(delta)+Fyf*std::cos(delta))-b*Fyr)/Iz;
    //dstate[0] = (a*Fyf*std::cos(delta)-b*Fyr)/Iz;
    dstate[0] = (a*Fyf-b*Fyr)/Iz;                       // dr

    //dstate[1] = 1/(m*Vx)
    //dstate[1] = 1/(m*Vx)*(Fyf*std::cos(delta-beta)+Fyr*std::cos(beta)-Fyr*std::sin(beta)-m*r*Vx);
