...
car_fblin::car_fblin(double P_dist)
{
    this->P_dist  = P_dist;
...
void car_fblin::control_transformation(double vPx, double vPy, double& v, double& phi)
{
...
void car_fblin::output_transformation(double& xP, double& yP)
{
    xP = x + P_dist*std::cos(theta);
    yP = y + P_dist*std::sin(theta);
}

void car_fblin::reference_transformation(double xref, double yref, double& xPref, double& yPref)
{
